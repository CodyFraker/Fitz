...
namespace Fitz.Api.Controllers.Rename
{
    [ApiController]
    [Route("api/rename")]
    public class GetRenamesController : ControllerBase
    {
        private readonly RenameService _renameService;
        private readonly FitzMetrics? _fitzMetrics;

        public GetRenamesController(RenameService renameService, FitzMetrics? fitzMetrics = null)
        {
        ...
        public IActionResult GetRenames([FromQuery] RenameStatusEnum? status)
        {
            ...
            try
            {
                var renames = _renameService.GetAllRenames(status);
