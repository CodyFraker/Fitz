...
namespace Fitz.Api.Controllers.Account.SetSafeBalance.Persistence;

public class SetSafeBalance(IDbContextFactory<BotContext> contextFactory, ILogger<SetSafeBalance> logger) : ISetSafeBalance
{
    private readonly IDbContextFactory<BotContext> _contextFactory = contextFactory;
    private readonly ILogger<SetSafeBalance> _logger = logger;
    ...
    {
        _logger.LogInformation("Updating account. UserId: {UserId}, SafeBalance: {SafeBalance}", account.Id, account.safeBalance);

        await using var context = await _contextFactory.CreateDbContextAsync(cancellationToken);
        context.Accounts.Update(account);
        await context.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Account updated successfully. UserId: {UserId}, SafeBalance: {SafeBalance}", account.Id, account.safeBalance);
    }
}
