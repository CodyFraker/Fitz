...
namespace Fitz.Api.Controllers.Account.SetLotterySubscribe.Persistence;

public class SetLotterySubscribe(IDbContextFactory<BotContext> contextFactory, ILogger<SetLotterySubscribe> logger) : ISetLotterySubscribe
{
    private readonly IDbContextFactory<BotContext> _contextFactory = contextFactory;
    private readonly ILogger<SetLotterySubscribe> _logger = logger;
    ...
    {
        _logger.LogInformation("Updating account. UserId: {UserId}, SubscribeToLottery: {SubscribeToLottery}", account.Id, account.subscribeToLottery);

        await using var context = await _contextFactory.CreateDbContextAsync(cancellationToken);
        context.Accounts.Update(account);
        await context.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Account updated successfully. UserId: {UserId}, SubscribeToLottery: {SubscribeToLottery}", account.Id, account.subscribeToLottery);
    }
}
