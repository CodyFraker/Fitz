...
{
    public class CalculateRenameCostControllerTests : IClassFixture<WebApplicationFactory<Fitz.Api.Program>>
    {
        private readonly WebApplicationFactory<Fitz.Api.Program> _factory;
        private readonly HttpClient _client;

        public CalculateRenameCostControllerTests(WebApplicationFactory<Fitz.Api.Program> factory)
        {
            _factory = factory.WithWebHostBuilder(builder =>
            {
                builder.ConfigureServices(services =>
                {
                    ...
                    {
                        options.UseInMemoryDatabase("CalculateRenameCostTestDb");
                    });
        ...
        [Fact]
        public async Task CalculateRenameCost_WithValidRequest_ReturnsOk()
        {
            ...
            var response = await _client.PostAsync("/api/rename/calculate-cost", content);

            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        }

        [Fact]
        public async Task CalculateRenameCost_WithInvalidUserId_ReturnsNotFound()
        {
            var request = new
            {
