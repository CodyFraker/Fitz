...
namespace Fitz.Features.Lottery
{
    public sealed class LotteryService
    {
        ...
        /// <returns></returns>
        public async Task EndLotteryAsync(Drawing currentLottery)
        {
            using IServiceScope scope = scopeFactory.CreateScope();
            using BotContext db = scope.ServiceProvider.GetRequiredService<BotContext>();
        ...
        public async Task<List<Winners>> DecideWinners(Drawing drawing)
        {
            using IServiceScope scope = scopeFactory.CreateScope();
            using BotContext db = scope.ServiceProvider.GetRequiredService<BotContext>();
        ...
        public async Task UpdateCurrentLottery(DateTime endDate)
        {
            ...
            Drawing drawing = this.GetCurrentDrawing();
        ...
        public int GetLastWinningTicket()
        {
            ...
            Drawing drawing = db.Drawing.Where((x) => x.CurrentLottery == false).OrderByDescending((x) => x.EndDate).FirstOrDefault();
